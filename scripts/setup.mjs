...
async function setup() {
  console.log("WristAgent Bridge setup wizard");
  console.log("");
...
async function runPreflight(values) {
  ...
  const checks = [
    ...
    await commandCheck("openclaw", ["--version"], "local agent CLI"),
    await commandCheck("tailscale", ["version"], "Tailscale CLI"),
    existsSync(defaultSessionsPath)
      ? okCheck("local agent sessions file", defaultSessionsPath)
      : warnCheck("local agent sessions file", `Not found at ${defaultSessionsPath}`),
  ];
...
async function detectMainSession(values) {
  ...
  if (!existsSync(sessionsPath)) {
    return { warning: "Could not auto-detect the local agent main session. The bridge will still start, but ask requests may fail until a target is configured." };
  }
  ...
  try {
    ...
    if (!main?.sessionId) {
      return { warning: "local agent sessions file exists, but no usable session id was found." };
    }
    return { sessionId: String(main.sessionId) };
  } catch (error) {
    return {
      warning: `Could not read local agent sessions file: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
...
function serializeEnv(values) {
  const lines = [
    "# Generated by WristAgent Bridge setup.",
    "# Keep this file private; it contains the iPhone pairing secret.",
  ];
...
function buildPairing(values) {
  ...
  return {
    ...payload,
    deepLink: `wristagent://pair?payload=${base64UrlJson(payload)}`,
  };
}
...
function appAvatarDataUri() {
  const logoPath = resolve(skillDir, "assets", "wristagent-app-logo.png");
...
<head>
  ...
  <title>WristAgent Pairing</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #0b0d12;
  ...
  <main>
    <section class="hero" aria-labelledby="title">
      <div class="brand">
        ${brandMark}
        <div>
          <p class="eyebrow">WristAgent</p>
          <p class="security">Private pairing over your Tailscale network</p>
        </div>
      </div>

      <div>
        <h1 id="title">Pair WristAgent with your bridge</h1>
        ...
        <div class="steps" aria-label="Pairing steps">
          ...
          <div class="step"><strong>2. Confirm</strong><span>The iPhone stores the pairing secret securely and runs diagnostics.</span></div>
    ...
    <section class="card" aria-label="Pairing code">
      <img class="qr" src="./pairing-qr.svg" alt="WristAgent pairing QR code">
      ...
      <p class="security">This page contains a pairing secret. Keep it local, do not publish it, and regenerate pairing if it is shared by mistake.</p>
    </section>
  </main>
</body>
</html>
...
function printSetup(values) {
  ...
  console.log("");
  console.log("WristAgent Bridge configured");
